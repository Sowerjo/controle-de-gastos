...
-- Recurring rules (recorrências) - MUST be created before transactions (FK)
CREATE TABLE IF NOT EXISTS recurring_rules (
  ...
  end_date DATE NULL,
  payment_status ENUM('pendente','pago','atrasado') NULL DEFAULT 'pendente',
  mode ENUM('automatic','manual') NOT NULL DEFAULT 'automatic',
  notify_days INT NOT NULL DEFAULT 3,
  last_run DATE NULL,
  created_at DATETIME NOT NULL,
  INDEX (user_id),
...
CREATE TABLE IF NOT EXISTS transactions (
  ...
  payment_status ENUM('pendente','pago','atrasado') NULL DEFAULT NULL,
  ...
  CONSTRAINT fk_tx_recurring FOREIGN KEY (recurring_rule_id) REFERENCES recurring_rules(id) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
